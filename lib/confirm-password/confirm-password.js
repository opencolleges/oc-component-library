...
export default class OCConfirmPassword extends React.Component {
  constructor(props) {
    ...
    this.MESSAGE_REQUIRED_FIELD = 'Field required';
    this.MESSAGE_PASSWORD_NOT_MATCHING = 'Password not matching';
    this.MESSAGE_REGEX_NOT_MATCHING = 'Password not meeting requirements';
    this.PASSWORD_PATTERN_REGEX = this.props.passwordPatternRegex
      ? new RegExp(this.props.passwordPatternRegex)
      : new RegExp('^[a]+$', 'g');
    this.PASSWORD_PATTERN_DISCLAIMER = this.props.passwordPatternDisclaimer
      ? this.props.passwordPatternDisclaimer
      : "Password must contains only a's (lower case letter)";
  }
