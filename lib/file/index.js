...
export default class OCFile extends React.Component {
  ...
  isValidFileSize = size => {
    if (size <= this.props.maxFileSize) return true;

    return false;
  };

  isValidFileType = name => {
    if (this.props.fileTypes.indexOf(getExtension(name)) !== -1) return true;
        ...
        if (addFile) {
          if (!this.isValidFileType(newFile.name)) {
            ...
            files.push({
              ...
              size: getBytes(newFile.size),
              progress: 0,
              message: `File exceeds ${getBytes(this.props.maxFileSize)}`,
              modifiers: 'progress--error'
            });
          } else {
            files.push({
