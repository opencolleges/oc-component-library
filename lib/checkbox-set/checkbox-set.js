...
export default class CheckboxSet extends React.Component {
  ...
  render() {
    ...
    let classNames = namespace('checkbox-set');

    state.error.length !== 0 && state.success.length !== 0
      ? (classNames += ` ${namespace(toModifier('error', 'checkbox-set'))}`)
      : state.error.length !== 0
      ? (classNames += ` ${namespace(toModifier('error', 'checkbox-set'))}`)
      : state.success.length !== 0 &&
        (classNames += ` ${namespace(toModifier('success', 'checkbox-set'))}`);
    ...
    return (
      <fieldset className={classNames} style={props.style}>
        <Grid modifiers="grid--gutter-x-fixed">
          {props.checkboxes.map((checkbox, index) => (
            <GridItem
              ...
              {props.cards ? (
                <Card modifiers="card--s card--clickable" tabIndex={false}>
                  <Checkbox
                    id={checkbox.id}
                    modifiers={
        ...
        <div className={namespace('checkbox-set__border')} />
        {(state.error.length !== 0 || state.success.length !== 0) &&
          props.message && (
            <span className={namespace('checkbox-set__message')}>
              {props.message}
            </span>
          )}
      </fieldset>
