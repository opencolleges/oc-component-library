...
import getCalendar from '../utilities/js/getCalendar';
...
export default class OCDate extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      calendar: getCalendar(this.props.selection ? this.props.selection : now),
  ...
  componentDidUpdate(previousProps, previousState) {
    ...
    if (this.state.selection !== previousState.selection) {
      this.setState({
        error: false,
        success: false
      });
      ...
      if (event.keyCode === 27) {
        ...
        if (this.state.selection) {
          this.setState({ active: false }, function() {
            this.setState({ calendar: getCalendar(this.state.selection) });
          });
        } else {
          this.setState({ active: false }, function() {
  ...
  render() {
    ...
    state.error && (hooks += 'date--error ');
    state.success && (hooks += 'date--success ');
      ...
      <div
        ...
        <span
          ...
          {!state.selection
            ...
            : getDateFormat(state.selection, 'Do MMM YYYY')}
        </span>
