...
export default class OCPagination extends React.Component {
  ...
  goToPage = pageNumber => {
    ...
    this.setState({ currentPage: pageNumber }, function() {
      paginationRef
        .getElementsByClassName(prefix('pagination__item--active'))[0]
        .focus();
      this.handleHighlight();
    });
  ...
  render() {
    ...
    return (
      <div
        className={prefix(
          `pagination ${state.mounted ? 'pagination--mounted' : ''}`
        )}>
              ...
              className={
                state.currentPage === page
                  ? getPageStatus(page)
                    ...
                    : prefix(
                        'pagination__item pagination__item--selectable pagination__item--active'
                      )
                  : getPageStatus(page)
                  ? prefix(
                      `pagination__item pagination__item--selectable ${getPageStatus(
