...
export default class OCRadioSet extends React.Component {
  ...
  render() {
    ...
    return (
      <fieldset className={prefix(`radio-set ${hooks}`)} style={props.style}>
        ...
        {(state.error || state.success) && (
          <span className={prefix('radio-set__message')}>
            ...
            {!props.message &&
              `! ${toTitleCase(state.selection)} ${(typeof state.selection ===
                'string' &&
                'is') ||
                (Array.isArray(state.selection) && 'are')} ${(state.error &&
                'incorrect') ||
                (state.success && 'correct')}.`}
          </span>
        )}
      </fieldset>
    );
